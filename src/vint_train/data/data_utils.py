...
def img_path_to_data(path: Union[str, io.BytesIO], image_resize_size: Tuple[int, int]) -> torch.Tensor:
    ...
    img = TF.to_tensor(Image.open(path))
    return img

def resize_and_aspect_crop_depth(
    depth: np.ndarray,
    ...
    # if w > h:
    #     th, tw = h, int(h * aspect_ratio)
    # else:
    #     th, tw = int(w / aspect_ratio), w

    # depth_t = TF.center_crop(depth_t, (th, tw))

    # depth_t = TF.resize(depth_t, image_resize_size, antialias=True)
