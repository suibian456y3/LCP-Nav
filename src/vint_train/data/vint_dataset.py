...
class ViNT_Dataset(Dataset):
    def __init__(
        ...
        self.depth_folder = data_folder + "_depth"
        ...
        self._load_index()
        self._build_caches()
        # Optional row-level goal overrides loaded from the submission
        # evaluation manifest.  When unset, training keeps the historical
        # stochastic goal sampler.
        self.fixed_goal_pairs = None
    ...
    def _build_caches(self, use_tqdm: bool = True):
        ...
        self._image_cache: lmdb.Environment = lmdb.open(cache_filename, readonly=True, lock=False)
    ...
    def _load_depth(self, trajectory_name, time) -> torch.Tensor:
        depth_path = get_data_path(self.depth_folder, trajectory_name, time, 'depth')
        ...
        depth_t = resize_and_aspect_crop_depth(depth, self.image_size)
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        if self.fixed_goal_pairs is None:
            f_goal, goal_time, goal_is_negative = self._sample_goal(f_curr, curr_time, max_goal_dist)
        else:
            try:
                f_goal, goal_time, goal_is_negative = self.fixed_goal_pairs[i]
            except KeyError as exc:
                raise KeyError(f"Missing fixed evaluation goal for dataset row {i}") from exc
        ...
        obs_image_depth = torch.tensor([])
        next_image_depth = torch.tensor([])
        goal_image_depth = torch.tensor([])
        ...
        if self.depth_img:
            obs_image_depth = torch.cat([
                self._load_depth(f, t) for f, t in context
            ])
            # next_image_depth = self._load_depth(f_curr, curr_time + 1)
            goal_image_depth = self._load_depth(f_goal, goal_time)
        ...
        if goal_is_negative:
            ...
            dis = min(goal_time - curr_time, 3)
        max_next_len = 3
        next_image_padded = torch.zeros((max_next_len, 3, 96, 96), dtype=torch.float32)
        next_mask = torch.zeros(max_next_len, dtype=torch.float32)
        next_times = list(range(curr_time + 1, curr_time + 1 + dis))
        next_content = [(f_curr, t) for t in next_times]
        if len(next_content) > 0:
            actual_images = [self._load_image(f, t) for f, t in next_content]
            next_image_padded[:dis] = torch.stack(actual_images)
            next_mask[:dis] = 1.0
        ...
        return (
            ...
            torch.as_tensor(obs_image_depth, dtype=torch.float32),
            torch.as_tensor(goal_image_depth, dtype=torch.float32),
            next_image_padded,
            # torch.as_tensor(next_image_depth, dtype=torch.float32)
            # None,
            next_mask
        )
